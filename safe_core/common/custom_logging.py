...
from safe_core.common.utilities import log_file_path
...
def setup_logger(logger_name, log_file=None, sentry_url=None):
    ...
    # (ariel): Make this log to /var/log/safe_core.log instead of
    #               /var/log/syslog
    ...
    if 'INASAFE_SENTRY' in os.environ:
        ...
        if add_logging_handler_once(logger, sentry_handler):
            logger.debug('Sentry logging enabled in safe_core')
    else:
        logger.debug('Sentry logging disabled in safe_core')
    # Set formatters
